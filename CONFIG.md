...
This plugin uses environment variables for configuration. You can set these in your Claude Code settings or shell environment. The server supports two platforms: **browser** (web automation) and **node** (Node.js backend debugging).
...
|----------|------|---------|-------------|
| `PLATFORM` | string | `browser` | `browser` for web automation, `node` for Node.js backend debugging |
...
## MCP in Docker Connecting to Another Container

When the MCP server runs in a Docker container and needs to connect to a Node.js app in another container (e.g. via `debug_connect` with `containerName`):

1. **Mount Docker socket** in the MCP container: `-v /var/run/docker.sock:/var/run/docker.sock`
2. **Docker CLI** must be available inside the MCP container
3. **host** parameter: Use `host.docker.internal` (or the app container name if on the same Docker network) to reach the inspector

Example `.mcp.json` for containerized setup:
